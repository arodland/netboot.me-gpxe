 ...
 * @v argv		Stack to store the expanded arguments
 * @ret rc		Return status
 *
 * The command is expanded, and its arguments are allocated with malloc()
 * and pushed onto a stack. The calling function should call free_stack_string
 * on the stack to free the arguments.
 */
